...
public class LevelExperienceOrbEntity extends Entity {
    ...
    public void tick() {
        ...
        if (!this.getEntityWorld().isSpaceEmpty(this.getBoundingBox())) {
        ...
        float vec3d2 = 0.98f;
        if (this.isOnGround()) {
            vec3d2 = this.getEntityWorld().getBlockState(this.getBlockPos().down()).getBlock().getSlipperiness() * 0.98f;
        }
    ...
    private void expensiveUpdate() {
        if (this.target == null || this.target.squaredDistanceTo(this) > 64.0) {
            this.target = this.getEntityWorld().getClosestPlayer(this, 8.0);
        }
        if (this.getEntityWorld() instanceof ServerWorld) {
            List<LevelExperienceOrbEntity> list = this.getEntityWorld().getEntitiesByType(TypeFilter.instanceOf(LevelExperienceOrbEntity.class), this.getBoundingBox().expand(0.5), this::isMergeable);
...
//    }
    ...
    public void onPlayerCollision(PlayerEntity player) {
        if (!this.getEntityWorld().isClient() && player.experiencePickUpDelay == 0 && this.orbAge > 20) {
