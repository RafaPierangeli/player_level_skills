...
public class TooltipUtil {
    ...
    private static void renderTooltip(MinecraftClient client, DrawContext context, List<Text> textList, @Nullable Identifier identifier, int x, int y) {
        ...
        for (Text text : textList) {
            if (i == 9) {
                context.drawText(client.textRenderer, text, x - maxTextWidth / 2 + (identifier != null ? 20 : 0), y + i, 0xFFFFFFFF, false);
            } else {
                context.drawText(client.textRenderer, text, x - maxTextWidth / 2, y + i + 8, 0xFFFFFFFF, false);
            }
            i += 10;
        }
